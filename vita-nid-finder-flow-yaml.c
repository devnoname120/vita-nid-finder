...
int main(int argc, char *argv[])
{
	...
	if (argc < 4) {
		//fprintf(stderr, "Usage:\n\t%s seg.bin module_name text_addr [isKernel]\n\n", argv[0]);
		return 1;
	}
	...
	while (i < mod_info->expBtm) {
		...
		if (export->lib_name) {
			...
			printf("\n");*/
			if (argc >= 5)
				printf("library %s 0x%08X %d\n", lib_name, export->module_nid, atoi(argv[4]));
			else
				printf("library %s 0x%08X\n", lib_name, export->module_nid);
